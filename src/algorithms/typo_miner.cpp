...
TypoMiner::TypoMiner(Primitives precise, Primitives approx)
...
void TypoMiner::AddMoreNeededOptions(std::unordered_set<std::string_view>& previous_options) const {
...
void TypoMiner::FitInternal(model::IDatasetStream& data_stream) {
    ...
    if (precise_pli != nullptr && approx_pli != nullptr) {
        ...
        boost::optional<boost::any> null_opt_any{is_null_equal_null_};
        TrySetOption(std::string{config::EqualNulls.GetName()}, null_opt_any, null_opt_any);

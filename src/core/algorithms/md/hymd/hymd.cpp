...
bool HyMD::TraverseLattice(bool traverse_all) {
    ...
    while (cur_level_ < lattice_.GetMaxLevel()) {
        for (model::LatticeNode const& node : lattice_.GetLevel(cur_level_)) {
            ...
            auto [new_rhs_sims, support] = lattice_.GetMaxRhsDecBounds(lhs_sims);
            ...
            for (size_t i = 0; i < col_matches_num; ++i) {
                ...
                if (!IsSupported(new_lhs_sims.value())) continue;
                for (size_t j = 0; j < col_matches_num; ++j) {
                    ...
                    if (rhs_sims[j] > new_lhs_sim) {
                        lattice_.AddIfMin({new_lhs_sims.value(), rhs_sim, j});
                    }
                }
            }
        }
...
bool HyMD::CheckRecordPair(size_t left_record, size_t right_record) {
    ...
    for (model::LatticeMd const& md : violated) {
        ...
        if (rec_rhs_sim >= rhs_min_similarities_[rhs_index] && rec_rhs_sim > md_lhs[rhs_index]) {
            lattice_.AddIfMin({md.lhs_sims, rec_rhs_sim, rhs_index});
        }
        for (size_t i = 0; i < column_matches_.size(); ++i) {
            ...
            if (md.rhs_sim > new_lhs.value()[md.rhs_index]) {
                lattice_.AddIfMin({new_lhs.value(), md.rhs_sim, md.rhs_index});
            }
        }
    }
